...
<style>
    ...
    .sortable li  {padding: 3px;margin: 2px;line-height: 30px;border-radius: 4px;}
			...
			if (is_array($this->getConfig('menu'))) {
				foreach ($menu AS $id => $item) {
					...
					<li id="itemlist_<?php echo $id ?>">
						<div class="inline-block image" style="width: 12%;box-sizing: border-box;background: url(<?php echo BASE_PATH . 'modules/mobileOldSchool/glyphish/glyphish-blue/' . $item['icon'] ?>.png) no-repeat;"><input type="hidden" name="menu[<?php echo $id ?>][icon]"  value="<?php echo $item['icon'] ?>" /></div>
...
<script>
	$(document).ready(function() {
		...
		<?php
		$imgs = '';
		foreach (glob("modules/mobileOldSchool/glyphish/glyphish-blue/*.png") as $filename) {
